...
class magikarp():
    def __init__(self, splash_frequency=3):
        self.splash_frequency = splash_frequency

    def run(self, agent):
        # Splashes like a magikarp every 5 seconds
        self.splash_time = not bool(agent.time % self.splash_frequency)
        if self.splash_time:
...
class ball_chase(atba):
    # An alias for the atba class
    pass


class goal_keep():
    # A defensive routine that will wait in net until it blocks a shot
    def run(self, agent):
        relative_target = agent.ball.location - agent.me.location
        local_target = agent.me.local(relative_target)
        defaultPD(agent, local_target)
